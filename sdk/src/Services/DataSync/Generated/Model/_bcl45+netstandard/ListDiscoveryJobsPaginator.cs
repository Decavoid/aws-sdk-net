...
namespace Amazon.DataSync.Model
{
    ...
    internal sealed partial class ListDiscoveryJobsPaginator : IPaginator<ListDiscoveryJobsResponse>, IListDiscoveryJobsPaginator
    {
        ...
        async IAsyncEnumerable<ListDiscoveryJobsResponse> IPaginator<ListDiscoveryJobsResponse>.PaginateAsync([System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
        {
            if (Interlocked.Exchange(ref _isPaginatorInUse, 1) != 0)
            {
                throw new System.InvalidOperationException("Paginator has already been consumed and cannot be reused. Please create a new instance.");
            ...
            {
                _request.NextToken = nextToken;
                response = await _client.ListDiscoveryJobsAsync(_request, cancellationToken).ConfigureAwaitEx();
                nextToken = response.NextToken;
